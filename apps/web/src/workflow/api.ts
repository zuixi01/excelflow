...
export interface Template { id: string; revision: number; status: string; updated_at: string; payload: { config: Config; version: number; confirmed: boolean; notes: string[]; error?: string; ai_model?: string; sample_count?: number; builtin?: boolean; tested_hash?: string; test_result?: {passed: boolean; records: number; errors: number; issues: Array<{row: number; field: string; message: string}>; artifacts?: Array<{name: string}>} } }
...
export interface ExportRun { id: string; status: string; created_at: string; payload: { revision: number; omitted_fields: string[]; error?: string; artifacts?: Array<{name: string; size: number; sheets: Array<{sheet: string; records: number}>}> } }
export interface Task { id: string; revision: number; status: string; updated_at: string; payload: { name: string; filename: string; config: Config; template_version: number; template_hash: string; stage: string; error?: string; record_count: number; error_count: number; sheets?: SourceSheet[]; ignored?: Array<{sheet: string; column?: string; reason: string}> }; exports: ExportRun[]; history: Array<{revision: number; created_at: string; actor: string; detail: {action: string; records?: number; changed_records?: number; added?: number; deleted?: number}}> }
